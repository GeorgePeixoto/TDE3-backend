...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($erros)) {
        try {
            $pdo = getConnection();
            $stmt = $pdo->prepare('INSERT INTO destinos (nome, pais, descricao, clima) VALUES (:nome, :pais, :descricao, :clima)');
            $stmt->execute([
                ':nome'      => $nome,
                ':pais'      => $pais,
                ':descricao' => $descricao,
            ...
            $_SESSION['msg']      = 'Destino criado com sucesso!';
            $_SESSION['msg_tipo'] = 'success';
            header('Location: index.php?page=destinos');
            exit;
        } catch (Exception $e) {
            $erros['db'] = 'Erro ao salvar o destino. Tente novamente.';
        }
    }
}
?>
...
<div class="container">
    ...
    <div class="form-card">
        <form method="POST" action="index.php?page=destinos_criar">
            <?= csrfInput() ?>
            <div class="form-group <?= isset($erros['nome']) ? 'has-error' : '' ?>">
                <label for="nome">Nome <span class="required">*</span></label>
                <input type="text" id="nome" name="nome" maxlength="150" value="<?= htmlspecialchars($nome) ?>" placeholder="Ex: Fernando de Noronha">
            ...
            <div class="form-group <?= isset($erros['pais']) ? 'has-error' : '' ?>">
                ...
                <input type="text" id="pais" name="pais" maxlength="100" value="<?= htmlspecialchars($pais) ?>" placeholder="Ex: Brasil">
            ...
            <div class="form-group <?= isset($erros['descricao']) ? 'has-error' : '' ?>">
                ...
                <textarea id="descricao" name="descricao" rows="4" placeholder="Descreva o destino..."><?= htmlspecialchars($descricao) ?></textarea>
            ...
            <div class="form-group <?= isset($erros['clima']) ? 'has-error' : '' ?>">
                ...
                <input type="text" id="clima" name="clima" maxlength="100" value="<?= htmlspecialchars($clima) ?>" placeholder="Ex: Tropical">
