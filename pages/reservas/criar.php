...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($erros)) {
        ...
        try {
            // Consulta vagas com lock para evitar race condition
            ...
            if ($vagas === false) {
                $pdo->rollBack();
                $erros['pacote_id'] = 'Pacote nao encontrado.';
            } elseif ((int) $vagas <= 0) {
                $pdo->rollBack();
                $erros['pacote_id'] = 'Este pacote nao possui vagas disponiveis.';
            } else {
                // Insere a reserva
                $stmt = $pdo->prepare(
                    'INSERT INTO reservas (cliente_id, pacote_id, data_reserva, status, valor_pago)
                     VALUES (:cliente_id, :pacote_id, :data_reserva, :status, :valor_pago)'
                );
                $stmt->execute([
            ...
            $pdo->rollBack();
            $erros['pacote_id'] = 'Erro ao processar a reserva. Tente novamente.';
        }
    }
}
...
<div class="container">
    ...
    <div class="form-card">
        <form method="POST" action="index.php?page=reservas_criar">
            <?= csrfInput() ?>
            ...
            <div class="form-group <?= isset($erros['valor_pago']) ? 'has-error' : '' ?>">
                ...
                <input type="text" id="valor_pago" name="valor_pago" value="<?= htmlspecialchars($valor_pago) ?>" placeholder="Preenchido ao selecionar o pacote">
